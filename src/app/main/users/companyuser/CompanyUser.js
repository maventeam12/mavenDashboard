...
import { getCompanyUser, newCompanyuser, resetCompanyUser, selectCompanyUserDB } from '../store/companyUserSlice';
...
function Service(props) {
  ...
  const service = useSelector(selectCompanyUserDB);
  const isMobile = useThemeMediaQuery((theme) => theme.breakpoints.down('lg'));
  const user = useSelector(selectUser);
  const routeParams = useParams();
  const { companyUserId } = routeParams;
