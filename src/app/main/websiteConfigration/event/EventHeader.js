...
import { removeEvent, saveEvent, addEvent } from '../store/eventSlice';
...
function EventHeader(props) {
  ...
  function handleSaveEvent() {
    ...
    if (!a) {
      dispatch(
        ...
        navigate('/websiteConfiguration/events');
      });
    } else {
      dispatch(
        saveEvent({
  ...
  function handleRemoveremoveEvent() {
    dispatch(
      removeEvent({
        access_token: window.localStorage.getItem('access_token'),
          email: user.email,
    ...
    <div className="flex flex-col sm:flex-row flex-1 w-full items-center justify-between space-y-8 sm:space-y-0 py-32 px-24 md:px-32">
      ...
      >
        <Button
          ...
          variant="contained"
          color="secondary"
          onClick={handleRemoveremoveEvent}
